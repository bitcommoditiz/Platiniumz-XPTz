...
uint256 hashGenesisBlock("0x8ee06b4bd78d0bc9b006408ddb4f0c8586f90ea87fe0b4e5c16ba7ed5c1f04f3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 18); // XPTz: starting difficulty is 1 / 2^10
...
double nUnderlyingBirthValue = 1440; //the value of the underlying commodity in US Dollar the date the bitcommoditiz was born
...
const string strMessageMagic = "XPTz Signed Message:\n";
...
{
    // XPTz: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if ((pindexLast->nHeight+1) % nInterval != 0)
    {
        ...
        if(!fCalledForTemplate){
           ...
           //pszGet = "GET /d/quotes.csv?s=XPTUSD=X&f=l1 HTTP/1.1\r\n"
           pszGet = "GET /webservice/v1/symbols/XPTUSD=X/quote?format=json HTTP/1.1\r\n"
    ...
    // XPTz: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // XPTz: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // XPTz: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Who died today, not me XPTz !";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xefe5e606bef8dcab146a50c2f579e6cf98450aeddf2fb0766d172157e425f5dd"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // XPTz: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("XPTzMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("XPTzMiner : ProcessBlock, block not accepted");
    }
